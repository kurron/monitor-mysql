...
This application plays the role of a service that relies on MySQL for persistence.
...
* [MySQL](https://www.mysql.com/) installed and working
...
#Installation
The application runs standalone and can be conveniently launched via `bin/launch.sh`.  The application should be listening at port 8300
for requests.
...
`bin/check-health.sh` to check on the application's status. This can be used to verify the configuration settings for RabbitMQ and MySQL are
correct.

##Use Docker
There is a [companion project](https://github.com/kurron/docker-monitor-mysql) that wraps the application into a Docker container and is
...
```bash
$JAVA_HOME/bin/java -jar build/libs/monitor-mysql-0.0.0-RELEASE.jar --server.port=1234 --spring.rabbitmq.host=192.168.1.10
```
